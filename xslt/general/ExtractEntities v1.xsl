...
<xsl:stylesheet xmlns:html="http://www.w3.org/1999/xhtml" xmlns:tei="http://www.tei-c.org/ns/1.0"
    ...
    <xsl:template match="tei:teiHeader">
        <xsl:copy>
            ...
            <!-- check if there is a profileDesc -->
            <xsl:choose>
                <xsl:when test="child::tei:profileDesc">
                    <xsl:apply-templates select="child::tei:profileDesc"/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="tei:profileDesc" name="tProfileDesc">
        <xsl:element name="tei:profileDesc">
            <xsl:apply-templates select="node()[not(self::tei:particDesc | self::tei:settingDesc)]"/>
